...
interface ProductBriefOptions {
    tools?: string[];
    noInteractive?: boolean;
}

export class ProductBriefCommand {
    async execute(options: ProductBriefOptions = {}): Promise<void> {
        ...
        console.log(chalk.blue('\n📋 Generating product-brief workflow files...\n'));
        ...
        // Generate product-brief workflow files for all configured tools
        let successCount = 0;
        let failCount = 0;
        ...
        for (const toolId of configuredTools) {
            ...
            if (configurator) {
                try {
                    // Generate only the product-brief workflow file
                    const body = configurator['getBody']('product-brief');
                    const relativePath = configurator['getRelativePath']('product-brief');
                    ...
                    const frontmatter = configurator['getFrontmatter']('product-brief');
        ...
        // Success message
        console.log(chalk.green(`\n✅ Product brief workflow files created successfully!\n`));
        console.log(chalk.blue('Next steps:'));
        console.log(chalk.gray('  1. Use /product-brief in your AI tool to create executive summaries'));
        console.log(chalk.gray('  2. Briefs will be saved to prompter/<slug>/product-brief.md\n'));
    }
